...
public class PlayerBhysics : MonoBehaviour {
    ...
    public bool RollingBasedOnInput = true;
    ...
    private void Start()
    {
        ...
        PreviousInput = sounds.transform.forward;
        Binput = GetComponent<PlayerBinput>();
    }
    ...
    void InputChecks()
    {
        ...
        {
            isRolling = true;
            if(!RollingBasedOnInput)
            Binput.lockInput = true;
        }
        if (Input.GetButtonUp("R1"))
        {
            isRolling = false;
            if(!RollingBasedOnInput)
            Binput.lockInput = false;
        }
    }
