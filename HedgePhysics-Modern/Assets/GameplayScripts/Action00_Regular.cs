...
public class Action00_Regular : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        if (Input.GetButton("Y") && Actions.Action06Control.ClosestRing != null)
        {
    ...
    void Update()
    {
        ...
        //Do Spindash
        if (Input.GetButtonDown("B") && Player.Grounded) { Actions.ChangeAction(3); Actions.Action03.InitialEvents(); }

        //Stomp
        if (Input.GetButtonDown("B") && !Player.Grounded) {
        ...
        if (Input.GetButton("X") && Actions.Action07.BoostBar.fillAmount > 0 && Player.Grounded) {
